...
// Separate interface for discount-capable payments
interface IDiscountCapable {
  calculateDiscount(amount: number): number;
  isDiscountEligible(productId: string): boolean;
}

// Specific payment interface - Inteface seggregating principle (ISP) in action
interface ICreditCardPayment extends IPaymentMethods, IDiscountCapable {
...
export class CreditCardPayment implements ICreditCardPayment {
  ...
  public async processPayment(paymentDetails: any) {
    console.log(
      `Processing credit card payments for ${paymentDetails.currency} ${paymentDetails.amount}`
    );
...
export class StripePayment implements IStripePayment {
  ...
  public async processPayment(paymentDetails: any): Promise<void> {
    console.log(
      `Processing Stripe payment for ${paymentDetails.currency} ${paymentDetails.amount}`
    );
...
export class PaypalPayment implements IPayPalPayment {
  ...
  public async processPayment(paymentDetails: any): Promise<void> {
    console.log(
      `Processing paypal payments for ${paymentDetails.currency} ${paymentDetails.amount}`
    );
    await new Promise((r) => setTimeout(r, 2000));
...
export class PaymentService {
  ...
  constructor(
    private readonly paymentFactory: IPaymentMethodFactory,
    paymentMethod: "credit_card" | "stripe" | "paypal"
  ) {
